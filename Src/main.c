...
* Version            : 1.0.0
* Description        : HSE Tuning via J-Link RTT
********************************************************************************
...
static void RTT_Read(void);
static void RTT_Debug(const char * message, uint8_t ret);
...
int main(void)
{
  ...
  SEGGER_RTT_printf(0, "BlueNRG-LPS HSE CALIB RTT v%s\r\n", VERSION);
...
void ModulesInit(void)
{
  ...
  if (ret != BLE_STATUS_SUCCESS) {
    RTT_Debug("Error in BLE_STACK_Init() 0x%02x\r\n", ret);
    while(1);
  }
}
...
void Device_Init(void)
{
  ...
  if(ret != 0) {
    RTT_Debug("Error in aci_hal_set_tx_power_level() 0x%04xr\n", ret);
    while(1);
  }
  ...
  if (ret != 0)
  {
    RTT_Debug("Error in aci_gap_init() 0x%04x\r\n", ret);
  }
  else
  {
    RTT_Debug("aci_gap_init() --> SUCCESS\r\n", ret);
  }
}
...
/**
  * @brief  J-Link RTT Debug: Output Debug Message
...
static void RTT_Debug(const char * message, uint8_t ret)
{
  ...
  if (ret != 0)
  {
    SEGGER_RTT_printf(0, message, ret);
  }
  else
  {
...
static void StartTone(void)
{
  if (tone_status == 0)
  {
    ...
    if (ret != 0)
    {
      RTT_Debug("Error in aci_hal_tone_start() 0x%04x\r\n", ret);
    }
    else
    {
      RTT_Debug("aci_hal_tone_start() --> SUCCESS\r\n", ret);
      tone_status = 1;
    }
  }
}
...
static void StopTone(void)
{
  if (tone_status == 1)
  {
    ...
    if (ret != 0)
    {
      RTT_Debug("Error in aci_hal_tone_stop() 0x%04x\r\n", ret);
    }
    else
    {
      RTT_Debug("aci_hal_tone_stop() --> SUCCESS\r\n", ret);
      tone_status = 0;
    }
  }
}
